        ...
        <main className="min-h-screen flex items-center justify-center p-4">
          <div className="max-w-md w-full bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-5">
              ...
              <div><label className="block text-sm font-bold text-gray-900 dark:text-gray-200 mb-1">Email Address</label><input type="email" required className="w-full border-2 border-gray-300 dark:border-gray-600 p-3 rounded bg-white dark:bg-gray-700 text-black dark:text-white font-medium outline-none focus:border-emerald-600" value={loginForm.email} onChange={(e) => setLoginForm({...loginForm, email: e.target.value})} /></div>
              <div><label className="block text-sm font-bold text-gray-900 dark:text-gray-200 mb-1">Password</label><input type="password" required className="w-full border-2 border-gray-300 dark:border-gray-600 p-3 rounded bg-white dark:bg-gray-700 text-black dark:text-white font-medium outline-none focus:border-emerald-600" value={loginForm.password} onChange={(e) => setLoginForm({...loginForm, password: e.target.value})} /></div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-200">
      ...
      <div className="max-w-[1400px] mx-auto p-4 md:p-8">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg dark:shadow-2xl border-t-4 overflow-hidden transition-colors" style={{ borderTopColor: branding.primary_color }}>
              ...
              <div className="p-6 md:p-8">
                <form onSubmit={handleSubmit} className="space-y-5">
                    <div><label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Title</label><input type="text" className="w-full border dark:border-gray-600 p-3 rounded bg-white dark:bg-gray-700 text-black dark:text-white font-medium focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="e.g. Fix loading dock door" value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} required /></div>
                    ...
                    <div><label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Problem Statement</label><textarea className="w-full border dark:border-gray-600 p-3 rounded bg-white dark:bg-gray-700 text-black dark:text-white h-24 focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="Describe the issue..." value={formData.problem_statement} onChange={(e) => setFormData({...formData, problem_statement: e.target.value})} /></div>
                    <div><label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Proposed Solution</label><textarea className="w-full border dark:border-gray-600 p-3 rounded bg-white dark:bg-gray-700 text-black dark:text-white h-24 focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="How do we fix it?" value={formData.proposed_solution} onChange={(e) => setFormData({...formData, proposed_solution: e.target.value})} /></div>
                    <div><label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase mb-1">Expected Benefit</label><input type="text" className="w-full border dark:border-gray-600 p-3 rounded bg-white dark:bg-gray-700 text-black dark:text-white focus:ring-2 focus:ring-emerald-500 outline-none" placeholder="e.g. Saves 10 mins/day" value={formData.expected_benefit} onChange={(e) => setFormData({...formData, expected_benefit: e.target.value})} /></div>
              ...
              <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border border-gray-100 dark:border-gray-700 transition-colors">
                <div className="flex items-center justify-between mb-4">
                    ...
                    {teamStats && currentUser && (
                        <span className="text-xs font-bold px-2 py-1 rounded bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-200">
                  ...
                  {myIdeas.map(idea => (
                    <div key={idea.id} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition flex justify-between items-center group">
                        <div className="border-l-4 pl-3" style={{ borderLeftColor: idea.status === 'Completed' ? '#10B981' : '#9CA3AF' }}>
                            <div className="font-bold text-gray-800 dark:text-gray-200 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">{idea.title}</div>
        ...
        {activeTab === "huddle" && (<div><div className="bg-green-600 dark:bg-green-700 text-white border-2 border-green-500 dark:border-green-800 p-6 rounded-xl shadow-md mb-8"><div className="flex justify-between items-start mb-4"><div><h2 className="text-2xl font-bold">Team Performance</h2><p className="text-gray-400 text-sm">Target vs. Actual Submissions</p></div>{currentUser.role === 'manager' && (<div className="flex gap-4"><select value={selectedTeamFilter} onChange={(e) => setSelectedTeamFilter(e.target.value)} className="bg-gray-700 text-white border border-gray-600 px-3 py-1 rounded text-xs font-bold outline-none"><option value="all">👁️ View All Teams</option>{companyStats.map(team => (<option key={team.id} value={team.id}>{team.name}</option>))}</select>{selectedTeamFilter !== 'all' && (<button onClick={() => setShowSettings(!showSettings)} className="bg-gray-700 hover:bg-gray-600 px-3 py-1 rounded text-xs font-bold border border-gray-600">⚙️ Set Goals</button>)}</div>)}</div>{showSettings && (<div className="bg-gray-800 p-4 rounded mb-4 border border-gray-700 animate-in fade-in slide-in-from-top-2"><label className="block text-xs font-bold text-gray-400 uppercase mb-2">Update Team Monthly Goal</label><div className="flex gap-2"><input type="number" style={{ color: '#ffffff', colorScheme: 'dark' }} className="bg-gray-700 border border-gray-500 px-3 py-1 rounded w-24 focus:ring-2 focus:ring-emerald-500 outline-none font-bold text-lg" value={newGoal} onChange={e => setNewGoal(parseInt(e.target.value))} /><button onClick={handleUpdateGoal} className="bg-emerald-600 text-white px-4 py-1 rounded text-sm font-bold hover:bg-emerald-500">Save</button></div></div>)}{teamStats ? (<div className="flex gap-8 items-center mt-6"><div className="flex-1"><div className="flex justify-between text-sm font-bold mb-2"><span>{teamStats.team?.name || "Loading..."}</span><span className="text-gray-400">Goal: {teamStats.team?.monthly_goal || 0}</span></div><div className="w-full bg-gray-700 rounded-full h-6 relative overflow-hidden"><div className={`h-6 rounded-full transition-all duration-1000 ${teamStats.teamActual >= teamStats.team?.monthly_goal ? 'bg-emerald-500' : 'bg-green-500'}`} style={{width: `${Math.min(100, (teamStats.teamActual / (teamStats.team?.monthly_goal || 1)) * 100)}%`}}></div></div></div>{selectedTeamFilter !== 'all' && (<div className="w-1/3 border-l border-gray-700 pl-8"><h3 className="text-xs font-bold text-gray-400 uppercase mb-2">Top Contributors</h3><div className="space-y-2">{teamStats.users?.map((u:any, idx:number) => (<div key={idx} className="flex justify-between text-sm"><span>{u.full_name}</span><span className={u.actual >= u.monthly_goal ? "text-green-400 font-bold" : "text-gray-400"}>{u.actual} / {u.monthly_goal}</span></div>))}</div></div>)}</div>) : <p className="text-gray-500 italic">Select a team to view detailed stats...</p>}</div><MatrixBoard ideas={ideas} users={users} onUpdate={updateIdea} onPromote={promoteIdea} isManager={currentUser.role === 'manager'} /><hr className="my-12 border-gray-200 dark:border-gray-700" /><h2 className="text-xl font-bold text-gray-800 dark:text-white mb-4">Execution Tracking</h2><KanbanBoard ideas={ideas} onUpdate={updateIdea} isManager={currentUser.role === 'manager'} /></div>)}
        {activeTab === "company" && (<div><div className="mb-8 text-center"><h2 className="text-3xl font-bold text-gray-900 dark:text-white">Company Leaderboard 🏆</h2><p className="text-gray-500 dark:text-gray-400">Monthly Performance by Team</p></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{companyStats.map((team, index) => (<div key={team.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-xl transition-shadow"><div className={`p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center ${index === 0 ? 'bg-yellow-50 dark:bg-yellow-900/20' : 'bg-white dark:bg-gray-800'}`}><h3 className="font-bold text-lg text-gray-800 dark:text-white">{index === 0 && '🥇 '} {index === 1 && '🥈 '} {index === 2 && '🥉 '}{team.name}</h3>{parseInt(team.submissions) >= team.monthly_goal && <span className="bg-green-100 text-green-700 text-xs font-bold px-2 py-1 rounded">On Target</span>}</div><div className="p-6 space-y-6"><div><div className="flex justify-between text-sm mb-1"><span className="text-gray-500 dark:text-gray-400 font-bold">New Ideas</span><span className="font-bold text-gray-900 dark:text-white">{team.submissions} <span className="text-gray-400 text-xs">/ {team.monthly_goal}</span></span></div><div className="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-3"><div className={`h-3 rounded-full ${parseInt(team.submissions) >= team.monthly_goal ? 'bg-emerald-500' : 'bg-green-600'}`} style={{width: `${Math.min(100, (team.submissions / team.monthly_goal) * 100)}%`}}></div></div></div><div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg flex justify-between items-center"><div><p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold">Completed</p><p className="text-2xl font-black text-gray-800 dark:text-white">{team.completions}</p></div><div className="text-3xl">✅</div></div></div></div>))}</div></div>)}
