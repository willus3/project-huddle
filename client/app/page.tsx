...
export default function Home() {
  ...
  const fetchBranding = () => {
      fetch("http://localhost:5000/settings/branding")
        .then(res => res.json())
        .then(data => {
  ...
  // Load Branding on Mount (Even before login)
  useEffect(() => {
      fetchBranding();
  }, []);
  ...
  const handleSaveBranding = async (e: React.FormEvent) => {
      e.preventDefault();
      await fetch("http://localhost:5000/settings/branding", {
  ...
  const handleEditUser = (user: any) => {
    ...
    setAdminUserForm({ full_name: user.full_name, email: user.email, password: "", role: user.role, team_id: user.team_id, monthly_goal: user.monthly_goal });
  };
      ...
      <main className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white p-8 rounded-xl shadow-lg border border-gray-200">
          ...
          <div className="flex justify-center mb-4">
              {branding.logo_url ? (
                  <img src={branding.logo_url} alt={branding.name} className="h-16 object-contain" />
              ) : (
                  <h1 className="text-3xl font-bold text-center text-gray-900 mb-2">{branding.name} 🚀</h1>
              )}
          </div>
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-5">
            ...
            <button type="submit" 
                ...
                className="w-full text-white font-bold py-4 rounded hover:opacity-90 transition shadow-md text-lg">
                Sign In
            </button>
          </form>
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      {/* NAV (DYNAMIC BRANDING) */}
      <nav 
        style={{ borderBottomColor: branding.primary_color }}
        className="bg-white border-b-4 px-8 py-4 flex justify-between items-center sticky top-0 z-50">
      ...
      <div className="max-w-[1400px] mx-auto p-8">
        ...
        {/* --- TABS 1, 2, 3 (DASHBOARD, HUDDLE, COMPANY) --- */}
        {activeTab === "dashboard" && (
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div className="divide-y divide-gray-100">
                  {myIdeas.map(idea => (<div key={idea.id} className="p-4 flex justify-between items-center"><div><div className="font-bold text-gray-800">{idea.title}</div><div className="text-xs text-gray-500 mt-1">{idea.status}</div></div></div>))}
        ...
        {activeTab === "admin" && currentUser.role === 'manager' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-200 lg:col-span-2">
                    ...
                    <p className="text-sm text-gray-500 mb-6">Customize the look of your Huddle Board.</p>
                    <form onSubmit={handleSaveBranding} className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="text" className="w-full border p-3 rounded bg-gray-50 text-black font-medium" 
                                value={branding.name} onChange={e => setBranding({...branding, name: e.target.value})} required />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Logo URL (Image Link)</label>
                            <input type="text" className="w-full border p-3 rounded bg-gray-50 text-black font-medium" 
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                <input type="color" className="h-12 w-12 border rounded cursor-pointer" 
                                    value={branding.primary_color} onChange={e => setBranding({...branding, primary_color: e.target.value})} />
                                <input type="text" className="w-full border p-3 rounded bg-gray-50 text-black font-medium uppercase" 
                                    value={branding.primary_color} onChange={e => setBranding({...branding, primary_color: e.target.value})} />
                            </div>
                        </div>
                        <div className="md:col-span-3 text-right">
                             <button type="submit" className="bg-gray-900 text-white font-bold py-3 px-8 rounded hover:bg-gray-800">Save Branding</button>
                        </div>
                    </form>
                </div>
                ...
                <div className="space-y-6">
                    <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-200">
                        ...
                        <form onSubmit={handleSaveTeam} className="space-y-4">
                            <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Team Name</label><input type="text" className="w-full border p-3 rounded bg-gray-50 text-black font-medium" value={adminTeamForm.name} onChange={e => setAdminTeamForm({...adminTeamForm, name: e.target.value})} required /></div>
                            <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Monthly Idea Goal</label><input type="number" className="w-full border p-3 rounded bg-gray-50 text-black font-medium" value={adminTeamForm.monthly_goal} onChange={e => setAdminTeamForm({...adminTeamForm, monthly_goal: parseInt(e.target.value)})} required /></div>
                            <div className="flex gap-2"><button type="submit" className="flex-1 bg-gray-900 text-white font-bold py-3 rounded hover:bg-gray-800">{editingTeamId ? 'Update Team' : 'Create Team'}</button>{editingTeamId && <button type="button" onClick={() => { setEditingTeamId(null); setAdminTeamForm({ name: "", monthly_goal: 10 }); }} className="px-4 py-3 bg-gray-200 text-gray-700 font-bold rounded">Cancel</button>}</div>
                        </form>
                    </div>
                    <div className="bg-white rounded-xl shadow border border-gray-200 overflow-hidden"><div className="bg-gray-50 px-6 py-4 border-b border-gray-200"><h3 className="font-bold text-gray-800">Existing Departments</h3></div><div className="divide-y divide-gray-100 max-h-[400px] overflow-y-auto">{companyStats.map(team => (<div key={team.id} className="p-4 flex justify-between items-center hover:bg-gray-50"><div><div className="font-bold text-gray-800">{team.name}</div><div className="text-xs text-gray-500">Goal: {team.monthly_goal}</div></div><button onClick={() => handleEditTeam(team)} className="text-xs bg-blue-50 text-blue-600 px-3 py-1 rounded font-bold hover:bg-blue-100">Edit</button></div>))}</div></div>
                </div>
                ...
                <div className="space-y-6">
                    <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-200">
                        ...
                        <form onSubmit={handleSaveUser} className="space-y-4">
                            <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Full Name</label><input type="text" className="w-full border p-3 rounded bg-gray-50 text-black font-medium" value={adminUserForm.full_name} onChange={e => setAdminUserForm({...adminUserForm, full_name: e.target.value})} required /></div>
                            <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Email (Login ID)</label><input type="email" className="w-full border p-3 rounded bg-gray-50 text-black font-medium" value={adminUserForm.email} onChange={e => setAdminUserForm({...adminUserForm, email: e.target.value})} required /></div>
                            <div className="grid grid-cols-2 gap-4"><div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Role</label><select className="w-full border p-3 rounded bg-gray-50 text-black" value={adminUserForm.role} onChange={e => setAdminUserForm({...adminUserForm, role: e.target.value})}><option value="employee">Employee</option><option value="manager">Manager</option></select></div>{!editingUserId && (<div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Password</label><input type="text" className="w-full border p-3 rounded bg-gray-50 text-black font-medium" value={adminUserForm.password} onChange={e => setAdminUserForm({...adminUserForm, password: e.target.value})} required={!editingUserId} /></div>)}</div>
                            <div><label className="block text-xs font-bold text-gray-500 uppercase mb-1">Assign to Team</label><select className="w-full border p-3 rounded bg-gray-50 text-black font-bold" value={adminUserForm.team_id} onChange={e => setAdminUserForm({...adminUserForm, team_id: e.target.value})} required><option value="">-- Select Department --</option>{companyStats.map(team => (<option key={team.id} value={team.id}>{team.name}</option>))}</select></div>
                            <div className="flex gap-2"><button type="submit" className="flex-1 bg-blue-600 text-white font-bold py-3 rounded hover:bg-blue-700">{editingUserId ? 'Update User' : 'Create User'}</button>{editingUserId && <button type="button" onClick={() => { setEditingUserId(null); setAdminUserForm({ full_name: "", email: "", password: "", role: "employee", team_id: "", monthly_goal: 5 }); }} className="px-4 py-3 bg-gray-200 text-gray-700 font-bold rounded">Cancel</button>}</div>
                        </form>
                    </div>
                    <div className="bg-white rounded-xl shadow border border-gray-200 overflow-hidden"><div className="bg-gray-50 px-6 py-4 border-b border-gray-200"><h3 className="font-bold text-gray-800">Employee Directory</h3></div><div className="divide-y divide-gray-100 max-h-[400px] overflow-y-auto">{users.map(u => (<div key={u.id} className="p-4 flex justify-between items-center hover:bg-gray-50"><div><div className="font-bold text-gray-800">{u.full_name} <span className="text-xs text-gray-400 font-normal">({u.role})</span></div><div className="text-xs text-gray-500">{u.email} • {u.team_name || "No Team"}</div></div><button onClick={() => handleEditUser(u)} className="text-xs bg-blue-50 text-blue-600 px-3 py-1 rounded font-bold hover:bg-blue-100">Edit</button></div>))}</div></div>
                </div>
            </div>
        )}
